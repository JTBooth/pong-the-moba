...
public class Pong extends BasicGame {
	...
	public Pong(String name, PongServer bankedServer, Player bankedPlayer1,
		...
		if (bankedPlayer1 != null) {
			player1 = bankedPlayer1;
		}

		if (bankedPlayer2 != null) {
			player2 = bankedPlayer2;
		}
	...
	public void init(GameContainer arg0) throws SlickException {
		...
		delayedEffects = Collections.newSetFromMap(new ConcurrentHashMap<DelayedEffect, Boolean>());
	...
	private void resetBall(int i) {
		ball.setPosition((float) (Settings.windowSize[0] / (2 * PTM_RATIO)),
				(float) (Settings.windowSize[1] / (2 * PTM_RATIO)));
		...
		if (i == 0) {
			...
			Log.error("You served the ball to not a player: player # " + i
					+ ". Players are 0 and 1.");
	...
	private void resetGame() {
		PongServer bankedServer = server;
		this.closeRequested();
		Settings.refreshSettings();
		Pong pong = new Pong("pong", server, player1, player2);
	}
	...
	private void makeWalls() {
		...
		SolidRect botWall = new SolidRect(width / 2, -0.01f, width, 0.005f,
				BodyType.KINEMATIC, getWorld(), this);
		...
		SolidRect topWall = new SolidRect(width / 2, (float) (height + 0.01),
				width, 0.005f, BodyType.KINEMATIC, getWorld(), this);
		rectRenderList.remove(topWall);
		topWall.getBody().getFixtureList().m_friction = 0;
	}

	private void makePaddles(int height, int width) {
		System.out.println("constructing paddles with length " + Settings.paddleLength);
		p1Paddle = new SolidRect(0.5f, height / 2, 0.2f, Settings.paddleLength,
				BodyType.KINEMATIC, getWorld(), this);
		player1.setPaddle(p1Paddle);
		p2Paddle = new SolidRect(width - 0.5f, height / 2, 0.2f,
				Settings.paddleLength, BodyType.KINEMATIC, getWorld(), this);
	...
	public Ball makeLaser(Player player) {
		...
		Laser laser = new Laser(x, y, Settings.laserRadius, direction,
				getWorld(), this);
		laser.getBody().getFixtureList().setDensity(Settings.laserDensity);
		return laser;
	}
